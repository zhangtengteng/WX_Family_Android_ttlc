 ...
 * @since  [产品/模块版本]
 */
public abstract class AbsRequest implements IHttpCallback
{
    /**
     * 请求方法
     */
     ...
     * @param httpUrl 请求地址
	 */
	public AbsRequest(final Context context, Handler handler, String httpUrl)
	{   
     ...
     */
	public AbsRequest(final Context context, Handler handler, String httpUrl, int methodType)
	{   
		this.context = context;
		this.handler = handler;
		if (METHOD_HTTP == methodType)
		{
			this.httpUrl = "http://" + WXApplication.getInstance().getRequestUrl() 
			                + HttpAddressProperties.PATH_PRE + httpUrl;
		}
		else
		{
			this.httpUrl = "https://" + WXApplication.getInstance().getRequestHttpsUrl() 
			                + HttpAddressProperties.PATH_PRE + httpUrl;
		}
	}
...
//	}
	...
	public void onTimeOut(int code, String message)
	{
		...
		if (this.handler != null)
		{
			handler.sendMessage(handler.obtainMessage(
			    CommontErrorCodes.NETWORK_TIMEOUT_ERROR, message));
		}
	}
	...
	public void onConnError(int code, String message)
	{
		...
		{
			handler.sendMessage(handler.obtainMessage(
			    CommontErrorCodes.NETWORK_ERROR, message));
		}
	}
