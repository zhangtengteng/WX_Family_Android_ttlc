...
public class WheelView extends ScrollView
{
    ...
    public static final int COLOR_SELECT = Color.parseColor("#34C385");
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            if (textView instanceof TextView)
            {
                if (i == position)
                {
                    ...
                    ((TextView) textView).setTextSize(TypedValue.COMPLEX_UNIT_SP,
                            TEXT_SIZE_SELECTED);
                    //                            TEXT_SIZE_NORMAL);
                    textView.setBackgroundColor(android.R.color.holo_red_light);
                }
                else
                {
                    ...
                    ((TextView) textView).setTextSize(TypedValue.COMPLEX_UNIT_SP,
                            TEXT_SIZE_NORMAL);
                    textView.setBackgroundColor(android.R.color.holo_green_light);
                }
            }
    ...
    private TextView createView(String title)
    {
        ...
        textView.setLayoutParams(new LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, dpToPx(TEXT_HEIGHT)));
        textView.setGravity(Gravity.CENTER);
        textView.setSingleLine();
        textView.setTextSize(TypedValue.COMPLEX_UNIT_SP, TEXT_SIZE_NORMAL);
        textView.setText(title);
        textView.setTextColor(COLOR_NORMAL);
        if (mItemHeight == 0)
        {
            //            mItemHeight = measureItemHeight(textView);
            mItemHeight = dpToPx(TEXT_HEIGHT);
            ...
            if (tempY - mInitY == 0)
            {
                ...
                else
                {
                    if (remain > mItemHeight / 2)
                    {
                        ...
                        WheelView.this.smoothScrollTo(0, (position + 1)
                                * mItemHeight);
                        onCallBack();
                    }
                    else
    ...
    private void onCallBack()
    {
        ...
        {
            mPickerListener.wheelSelect(mSelection - mOffset,
                    mItems.get(mSelection));
        }
    }
